...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*susunhelp/i.test(m.quoted.text)) return !0
    ...
    if (m.quoted.id == this.susunkata[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!*\n+${this.susunkata[id][2]} XP`, '© 2022 - Zulfikar Bot', { 'button[0]': 'Susun Kata', 'row[0]': '.susunkata' }, m)
