...
async function handler(m, { command, usedPrefix }) {
    ...
    switch (command) {
        ...
        if (!room) {
                await this.sendButton(m.chat, 'Kamu tidak sedang berada di anonymous chat', wm, null, { 'button[0]': 'Search Partner', 'row[0]': '.start' }, m)
                throw 0
            }
       ...
       //     if (!room) throw 'Kamu tidak sedang berada di anonymous chat\nKetik #start\nKetik /start\nKetik !start'
            this.sendButton(m.chat, 'Kamu telah memberhentikan chat', wm, null, { 'button[0]': 'Search Partner', 'row[0]': '.start' }, m)
            let other = room.other(m.sender)
            if (other) this.sendButton(other, 'Partner telah memberhentikan chat', wm, null, { 'button[0]': 'Search Partner', 'row[0]': '.start' }, m)
