...
END:VCARD
   ...
   //, { quoted, ...options }
    ...
    /**
         ...
         * @param {Object} options
         */
    conn.sendButtonLoc = async(jid, urls, content, footer, list = {}, quoted, options = {}) => {
      let buffer
      if (urls) try { buffer = (await conn.getFile(urls)).data } catch { buffer = null }
      let data = JSON.parse(JSON.stringify(list))
      const buttonList = []
      for(let i=1; i<Object.keys(list).length;i++){
      	buttonList[i-1] = {buttonId: data['row['+i+']'], buttonText: {displayText: data['button['+i+']']}, type: 1}
      }
      try{ return await conn.sendMessage(jid, {
        location: { jpegThumbnail: buffer },
        text: content,
        footer: footer,
        buttons: buttonList, 
        headerType: 6
      }, { quoted, ...options }) } catch (e) { conn.sendMessage(jid, e, { quoted }) }
    }
